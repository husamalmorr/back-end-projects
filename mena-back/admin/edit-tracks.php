...
$id=$_GET['id'];
	$query=mysqli_query($conn,"select * from tracksitems where id='$id'");
...
if (isset($_POST['save'])) {
 ...
 $category = $_POST['category'];
	...
	$folder = "../images/" . $filename;
	 ...
	 if($filename!=''){
 		    $sql = "update tracksitems set item='$item',category='$category',status='$status',image='$filename',lang='$lang' where id='$id'";
			   mysqli_query($conn, $sql);
				$msg ="<script> alert('successfull updated'); window.location.href='./tracks.php'</script>";
echo $msg;
    // Execute query
	  ...
	  if (move_uploaded_file($tempname, $folder)) {
       // echo "<h3>  Image uploaded successfully!</h3>";
        ...
        echo "<h3>  Failed to upload image!</h3>";
		  ...
		  $sql = "update tracksitems set item='$item',category='$category',status='$status',lang='$lang' where id='$id'";
		   mysqli_query($conn, $sql);
				$msg ="<script> alert('successfull updated'); window.location.href='./tracks.php'</script>";
echo $msg;
    // Execute query
  ...
  <div class="modal-dialog" role="document">
	...
	<form method="post" enctype="multipart/form-data"> 
      <div class="modal-header text-center">
        <h4 class="modal-title w-100 font-weight-bold">Edit tracks</h4>
