<?php include("controllers/tracks-title-control.php");
session_start();
 	  if($_SESSION['id']==0)
	  {
...
<tr class="header">
  ...
  <td> <?php echo $i++;?> </td>
    ...
    <td><a href="edit-tracks-title.php?id=<?php echo $row['id'];?>"> <i class="fa fa-edit"> </i> </a> </td>
	<?php   if(mysqli_num_rows($query)>1){?>
		  		    <td><a href="tracks-title.php?id=<?php echo $row['id'];?>&del=delete" onClick="return confirm('are you want to delete user?')"> <i class="fa fa-trash"> </i> </a></td>
	<?php }?>
<?php  if(isset($_GET['del']))
		  {if(mysqli_num_rows($query)>1){
		          $query4=mysqli_query($conn,"delete from trackstitle  where id ='".$_GET['id']."'");
				  	$msg ="<script> alert('successfull deleted'); window.location.href='./tracks-title.php'</script>";
