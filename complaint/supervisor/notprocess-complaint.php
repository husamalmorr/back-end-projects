...
$sql=mysqli_query($con,"select regDate from tblcomplaints where date>='$date' and status is null ");
while ($rw=mysqli_fetch_array($sql)) {
	$date_=$rw['regDate'];
...
$currentTime = date( 'H:i:s  A', time () );
$date=date('Y-m-d G:i:s A');
$query=mysqli_query($con,"select tblcomplaints.*,users.fullName as name from tblcomplaints join users on users.id=tblcomplaints.userId where tblcomplaints.status is null and tblcomplaints.state='$st6'and tblcomplaints.date>='$date'");
}
...
$currentTime = date( 'H:i:s  A', time () );
$date=date('Y-m-d G:i:s A');
$query=mysqli_query($con,"select tblcomplaints.*,users.fullName as name from tblcomplaints join users on users.id=tblcomplaints.userId where tblcomplaints.status is null and tblcomplaints.state='$st7'and tblcomplaints.date>='$date'");
}
 }
