...
$st='Time Finished';
$sql=mysqli_query($con,"select DISTINCT regDate from tblcomplaints where date<='$date' ");
while ($rw=mysqli_fetch_array($sql)) {
	$date_=$rw['regDate'];
...
color: white;">  اسم و رقم الطالب </th>
											   <th style="display:none"> رقم الطالب </th>
											    <th colspan="3" style="background:#1c2282de;
...
$st='Time Finished';
$st2="قسم الاي تي";
$query=mysqli_query($con,"select tblcomplaints.*,users.fullName as name from tblcomplaints join users on users.id=tblcomplaints.userId where  tblcomplaints.state='$st2'and tblcomplaints.date<='$date' ");
$query2=mysqli_query($con,"UPDATE tblcomplaints set status='$st'  where  tblcomplaints.date<='$date'");
}
										...
										<tr>
											...
											$replace='ارسال تنبيه';									      
											 ...
											 if( $date< date('Y-m-d G:i:s A')){
													  echo "<td colspan='3'><button><a href=http://localhost/compliant/admin/send-feedback.php?cid=".$cid.">".$replace."</a></button></td>";
